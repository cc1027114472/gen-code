...
func New(base logger.Logger, appName string, trustedProxies []string, httpAccess bool, handlers Handlers) (*gin.Engine, error) {
	...
	if handlers.Runtime != nil {
		...
		api.GET("/workspaces", handlers.Runtime.Workspaces)
		api.POST("/workspaces", handlers.Runtime.CreateWorkspace)
		api.POST("/workspaces/:id/activate", handlers.Runtime.ActivateWorkspace)
		...
		api.POST("/threads/:id/preferences", handlers.Runtime.UpdateThreadPreferences)
