...
`use-my-browser` 是一个浏览器自动化策略 skill，适用于需要操作用户当前浏览器会话、检查页面、承接 DevTools 上下文、调试动态应用、操作已登录网站，以及从渲染后的页面中提取媒体内容的智能体。它还会帮助智能体判断何时真正需要浏览器，以及何时应将任务路由到公共网页工具、实时 Chrome 会话、原始抓取或干净的浏览器上下文。
...
| 能力 | 含义 |
| --- | --- |
| 智能工具路由 | 智能体可以根据任务在公共网页搜索、处理后读取、原始抓取、基于 shell 的抓取和实时浏览器工具之间做出选择，而不是将所有事情都强行塞进同一条路径。 |
...
该 skill 不只是一套浏览器技巧的集合。它教授的是一种浏览策略。
