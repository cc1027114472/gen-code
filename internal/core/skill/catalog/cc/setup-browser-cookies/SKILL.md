...
- 如果 `SKILL_PREFIX=true`，跨 skill 建议或调用时使用 `/gstack-` 前缀，磁盘路径规则不变。
- 如果输出里出现 `UPGRADE_AVAILABLE` 或 `JUST_UPGRADED`，按原 gstack 升级流程处理。
- telemetry、Completeness Principle、routing rules 和 contributor mode 等一次性初始化逻辑，继续按 preamble 中的规则处理。
