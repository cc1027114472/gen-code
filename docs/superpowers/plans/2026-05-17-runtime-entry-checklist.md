...
- canonical browser acceptance now explicitly includes a controlled browser interaction lane for `browser.open`, `browser.type`, `browser.click`, `browser.extract`, and `browser.screenshot`, plus one authenticated controlled-session fixture lane and one constrained verified HTTPS read-only lane
...
- authenticated browser fixture task IDs, session evidence, and screenshot artifact path
- public-web read-only browser task IDs, target URL, and screenshot artifact path
